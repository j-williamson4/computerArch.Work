...
void CPU::decode() {
  ...
  switch(opcode) {
    ...
    case 0x04: D(cout << "beq " << regNames[rs] << ", " << regNames[rt] << ", " << pc + (simm << 2)); 
              ...
              aluSrc1 = pc;
              writeDest = false;
              actualTaken = false; // default not taken
              pipeStats.registerSrc(rt, ID);
              pipeStats.registerSrc(rs, ID);
              pipeStats.countBranch();
              targetPC = pc + (simm << 2); // easier to pass to update function
              prediction = branchPredictor.predict(pc); // predict whether branch will be taken / not taken

              if( regFile[rs] == regFile[rt] ) // Branch is taken
              {
                actualTaken = true;
                flush = branchPredictor.update(pc, targetPC, prediction, actualTaken);
                pc += (simm << 2);
                pipeStats.countTaken();
                if(flush == true) // if targetPC != predictionTable.targetPC
                  pipeStats.flush(ID);
                if(prediction == false) { // predicted not taken, branch was taken
                  pipeStats.flush(ID);
                }  
              }
              else // Branch is not taken 
              {
                if(prediction == true) { // predicted taken, actually wasnt taken
                  pipeStats.flush(ID);
                }
                flush = branchPredictor.update(pc, targetPC, prediction, actualTaken);
              }
    ...
    case 0x05: D(cout << "bne " << regNames[rs] << ", " << regNames[rt] << ", " << pc + (simm << 2)); 
              ...
              aluSrc1 = pc;
              writeDest = false;
              actualTaken = false; // default to not actually taken
              pipeStats.registerSrc(rt, ID);
              pipeStats.registerSrc(rs, ID);
              pipeStats.countBranch();
              targetPC = pc + (simm << 2); // easier when passsing to update function
              prediction = branchPredictor.predict(pc); // predict whether branch will be taken/not-taken

              if (regFile[rs] != regFile[rt]) // Branch is taken
              {
                actualTaken = true; // branch is actually taken
                flush = branchPredictor.update(pc, targetPC, prediction, actualTaken); // update counter / targetPC
                pc += (simm << 2); // jump to target pc
                pipeStats.countTaken();
                if(flush == true) // if predictionTable's pc != targetPC, target misprediction
                  pipeStats.flush(ID);
                if(prediction == false) // predicted not taken, actually was taken
                  pipeStats.flush(ID);
              }
              else // Branch is not taken
              {
                if(prediction == true) { //direction misprediction
                  pipeStats.flush(ID);
                }
                flush = branchPredictor.update(pc, targetPC, prediction, actualTaken);
              }
...
void CPU::printFinalStats() {
  ...
  cout << "Flushes: " << pipeStats.getFlushes() << endl;
  cout << "Stalls: " << pipeStats.getStalls() << endl << endl;
 
  branchPredictor.printFinalStats();
}
